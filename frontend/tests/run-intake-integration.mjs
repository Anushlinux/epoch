...
async function start() {
  child = spawn(path.join(backend, '.venv/bin/epoch-backend'), ['serve'], {
    cwd: backend,
    env: { ...process.env, EPOCH_DATA_DIR: data, EPOCH_PORT: String(port), EPOCH_HOST: '127.0.0.1', EPOCH_CORS_ORIGINS: JSON.stringify([browserOrigin]), EPOCH_LOG_LEVEL: 'warning' },
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    if (child.exitCode !== null) throw new Error(`Backend exited: ${serverLog}`);
    try { if ((await fetch(`${origin}/api/health`)).ok) return; } catch { /* bounded startup poll */ }
...
const http = async (route, body, headers = {}) => {
  const response = await fetch(origin + route, { method: body ? 'POST' : 'GET', headers: { ...(body ? { 'Content-Type': 'application/json' } : {}), ...headers }, ...(body ? { body: JSON.stringify(body) } : {}) });
...
try {
  ...
  const created = await http('/api/tasks', payload, { Origin: browserOrigin }); assert.equal(created.status, 201); assert.equal(created.body.status, 'pending');
  const retry = await http('/api/tasks', payload, { Origin: browserOrigin }); assert.equal(retry.status, 200); assert.deepEqual(retry.body, created.body);
  const normalized = await http('/api/tasks', { ...payload, message: ` ${payload.message} ` }); assert.equal(normalized.status, 200);
  const conflict = await http('/api/tasks', { ...payload, message: 'Changed under the same ID' }); assert.equal(conflict.status, 409);
  const invalid = await http('/api/tasks', { ...payload, client_request_id: crypto.randomUUID(), message: ' ' }); assert.equal(invalid.status, 422);
  const missing = await http(`/api/tasks/${crypto.randomUUID()}`); assert.equal(missing.status, 404);
  const denied = await http('/api/tasks', { ...payload, client_request_id: crypto.randomUUID() }, { Origin: 'http://localhost:59999' }); assert.equal(denied.status, 403);
  const preflight = await fetch(`${origin}/api/tasks`, { method: 'OPTIONS', headers: { Origin: browserOrigin, 'Access-Control-Request-Method': 'POST', 'Access-Control-Request-Headers': 'content-type' } });
  assert.equal(preflight.status, 200); assert.equal(preflight.headers.get('access-control-allow-origin'), browserOrigin);
  assert.equal((await http('/api/tasks?limit=20&offset=0')).body.total, 1);
  await stop(); await start();
  const restored = await http(`/api/tasks/${created.body.id}`); assert.deepEqual(restored.body, created.body);
  const restartedRetry = await http('/api/tasks', payload); assert.equal(restartedRetry.status, 200);
  console.log('Real HTTP proof passed: health, 201/200 identical + normalized retry, 409/422/404/403, CORS, list/detail and restart persistence.');
  const result = await new Promise((resolve, reject) => {
    const tests = spawn(process.execPath, [path.join(frontend, 'node_modules/@playwright/test/cli.js'), 'test', '--config', 'playwright.intake.config.mjs'], { cwd: frontend, env: { ...process.env, EPOCH_INTAKE_TEST_ORIGIN: origin }, stdio: 'inherit' });
