...
Earlier completion checkpoint (superseded by final remediation below): missing-detail reconnect and server-scope notices fixed; 37 state tests, 26 fixture browser tests and 12 actual-backend browser checks passed. HTTP/restart proof and screenshots refreshed; only the documented AO CORS restriction and future execution phases remain blocked. All completed frontend work is included in PR #2.

## Required final remediation

1. Unify fixture checkpoint evidence validation across events/reconnect/scope transitions; require the exact checkpoint ID set on same-scope reconnect and reject duplicate IDs or missing failed/needs-input evidence.
2. Block disconnected fixture request creation at controls and command dispatch. Derive delivery presentation from explicit outcome/checkpoint ordering without changing the supplied task verdict; add regressions for changes after delivery.
3. Add a minimal Node dev command at `127.0.0.1:5173`, consume the unchanged backend at its allowed origin, and switch integration tests to the actual served frontend. Open that app through AO, verify real intake and retain a usable isolated preview.
4. Update evidence/limitations and PR #2, push all fixes, and report to epoch-2 without merge or backend edits.

Final remediation verified: 41 state tests, 30 fixture browser tests, and 12 normally served intake browser tests passed, along with actual HTTP and backend-restart checks. AO connected through the supported dev URL, saved a request, reloaded its detail and matched the stored task through independent HTTP reads. Screenshots and receipt evidence are retained. Both isolated preview servers remain running; later execution/repair phases remain blocked. Commit/push and PR handoff complete the delivery.
