...
- `npm run test:all --prefix frontend`: **35 state tests and 26 fixture browser tests passed** (11 new intake adapter/state tests plus the original 24 state tests).
- `npm run test:integration --prefix frontend`: actual local HTTP checks and **10 Chromium intake browser tests passed**, five cases each on 1440px desktop and 390px mobile emulation. The unchanged installed backend CLI used a random loopback port and temporary SQLite data, then was stopped and its data removed.
